...
const ViewAccessory = () => {
  ...
  const fetchAccessoryData = async (accessoryId: string) => {
    try {
      const accessoryData = await AccessoriesService.fetchSingleAccessoryWithImages(accessoryId);
      ...
      setError(error.message || "Error in Viewing Accessory");
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetchAccessoryData(id as string) as IAccessory;
        setAccessory(result);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <>
      <MetaComponent title={accessory.Meta_Title__c} />
